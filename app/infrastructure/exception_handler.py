...
from app.core.application.exceptions import ApplicationError
...
async def app_error_handler(request_: Request, exc: ApplicationError, status_code: int) -> Response:  # noqa: ARG001
    return await handle_error(
        err_data=ErrorData(message=exc.message, code=exc.code),
        status_code=status_code,
    )


async def unknown_exception_handler(request_: Request, exc_: Exception) -> Response:  # noqa: ARG001
    return await handle_error(err_data=ErrorData(message="Unknown error occurred", code=1), status_code=500)
